...
/// <summary>
/// Manages how the enemy controller plays.
/// This includes the deck and kind of AI to use.
/// </summary>
public class EnemyController : MonoBehaviour {
    ...
    private List<CardInfo> cardsInHand = new List<CardInfo>();
    public int startHandSize;

    private void Awake() {
        instance = this;
        foreach (CardInfo info in cardPackToUse.possibleCardsList) {
            deckToUse.Add(info);
        }
    }
    ...
    private void RandomizeAIType() {
        AIType[] values = (AIType[])System.Enum.GetValues(typeof(AIType));
        enemyAIType = values[Random.Range(0, values.Length)];
    }
    ...
    IEnumerator EnemyActionCo() {
        ...
        if (enemyAIType == AIType.placeFromDeck || enemyAIType == AIType.handRandomPlace) {
            ...
            // dont place if it already has a card
        ...
        switch (enemyAIType) {
            ...
            case AIType.handRandomPlace:
                ...
                while (selectedCard != null && iterations > 0 && selectedPoint.activeCard == null) {
                    ...
                    // checks if we can play a second card if theres enough mana
                    selectedCard = SelectedCardToPlay();
                    iterations--;
    ...
    public void PlayCard(CardInfo cardInfo, CardPlacePoint placePoint) {
        Card newCard = Instantiate(cardToSpawn, cardSpawnPoint.position, cardSpawnPoint.rotation);
        ...
        cardsInHand.Remove(cardInfo);

        BattleController.instance.SpendEnemyMana(cardInfo.manaCost);
    }

    private CardInfo SelectedCardToPlay() {
        CardInfo cardToPlay = null;

        List<CardInfo> cardsToPlay = new List<CardInfo>();

        foreach (CardInfo card in cardsInHand) {
